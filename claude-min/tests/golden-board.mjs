...
// on purpose and then regenerates with --update. `sup` is what an external
// supervisor would look like; on Claude Code the script never has one, so the
// `idle` case (sup null, costs summed from the attempts) is the real one.
...
const { renderBoard, boardJson } = await import(path.join(SRC, ".claude", "lib", "board.mjs"))
...
const models = JSON.parse(readFileSync(path.join(SRC, ".claude", "models.json"), "utf8"))
...
const supBusy = { sessionId: "ses_fixed", title: "Desarrollo profesional de visor de registros", slug: "quiet-knight", last: { at: T(1), kind: "tool", text: "bash: node .claude/codegen.mjs status" }, waiting: null, userMessages: [{ id: "m1", at: T(30), text: "resume el estado y dime qué falta" }] }
