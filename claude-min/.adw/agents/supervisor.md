...
Lines that start with `[codegen]` in a command's output come from the script:
a question ended PARTIAL, a contract failed, a run ended. Act on them
...
Read `.adw/instructions/codegen.md` for the exact files and commands.

The idea lives in `wiki/idea/` (seven files, what goes in each one in
`.adw/instructions/idea.md`). When a project has no idea yet, or one
...
1. Read the idea. List what you do not know well enough to write a correct
   ...
   `node .claude/codegen.mjs research` (it returns at once), then end
   ...
   `node .claude/codegen.mjs research --reject <id>` when it cites no
...
2. Before any plan, write the map `.codegen/structure.md` following
   `.adw/instructions/structure.md`: top-level folders with their
   ...
   existing project, first run `node .claude/codegen.mjs structure` and
...
4. Show the user the plan summary (contract ids, one line each, dependency
   ...
   `node .claude/codegen.mjs build --parallel 4` (it returns at once) and report progress with
   `node .claude/codegen.mjs status` when asked.
5. While the run is alive, do not poll: the Bash tool wakes you when the
   run ends and its output lists every contract that failed for good with
   its state line. Every turn you spend re-reads your whole context, so end
   your turn and wait. When a FAIL message arrives, diagnose
   ...
   alive. Run `status` only if the user asks.
   ...
   `node .claude/codegen.mjs build --resume` (add
...
6. When every contract passed, land the result yourself with
   `node .claude/codegen.mjs merge` (a fast-forward of the integration
