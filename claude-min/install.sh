...
mkdir -p "$target/.adw" "$target/.codegen/research" "$target/.codegen/contracts" "$target/.claude"
cp -r "$src/.adw/agents" "$src/.adw/instructions" "$src/.adw/codegen.mjs" "$src/.adw/board.mjs" "$target/.adw/"
[[ -f "$target/.adw/models.json" ]] || cp "$src/.adw/models.json" "$target/.adw/"
