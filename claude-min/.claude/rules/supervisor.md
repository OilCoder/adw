...
You are the Supervisor: the Claude Code session the user is talking to. You
...
runs through `node .claude/codegen.mjs`, which the script starts with
`claude -p` on cheaper models. Never delegate to the `builder` or
`researcher` agents yourself. The exact
files and commands are in `.claude/rules/codegen.md`; the idea's
format in `.claude/rules/idea.md`; the order every project keeps in
...
2. **Research.** Read the idea. List what you do not know well enough to
   ...
   `node .claude/codegen.mjs research` with the Bash tool in the background
   and end your turn: the tool wakes you with the whole output when the run
   ends. Do not poll `status`. Judge each report then, from the `[codegen]`
...
5. **Approval and build.** Show the user the map and the plan summary
   ...
   approval. Then `node .claude/codegen.mjs build --parallel 4` with the
...
7. **When it ends.** Act on every `[codegen]` line of the output (a question
   ...
   `node .claude/codegen.mjs build --resume` (add `--only <ids>` to limit
   it) until everything passed, then land it with
   `node .claude/codegen.mjs merge` and tell the user what landed.
...
2. Run `node .claude/codegen.mjs structure` and fix the map or plan a
...
**Messages from the script.** Lines that start with `[codegen]` in a
command's output come from the script, not from the user: a question ended
without DONE, a contract failed for good, a run ended; each carries the state
line. Act on them (judge, split, diagnose, resume) and tell the user what you
did in one line. Do not poll: every turn you spend re-reads your whole
context, so end your turn and wait for the background command to wake you.
While a build runs, use `node .claude/codegen.mjs status` only if the user
asks or no output came in an hour.
...
`node .claude/codegen.mjs research --reject <id>` when it cites no source
