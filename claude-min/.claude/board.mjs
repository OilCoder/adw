...
export function boardData({ root, sandboxes, plan, report, current, research, alive, researchRun, researchAlive, events = [], now = Date.now() }) {
  ...
  const sup = null
  // Cost and wall time come from the attempts themselves (the `result` event
  // of every claude -p run): API-equivalent dollars, since the subscription
  // charges nothing per call.
  const costs = (() => {
    const byRole = { builder: 0, researcher: 0 }, byModel = { builder: {}, researcher: {} }, byContract = {}
    let total = 0
    const add = (role, model, cost, ms, id) => { const m = (byModel[role][model] ??= { sessions: 0, cost: 0, ms: 0 }); m.sessions++; m.cost += cost ?? 0; m.ms += ms ?? 0; byRole[role] += cost ?? 0; total += cost ?? 0; if (id) byContract[id] = (byContract[id] ?? 0) + (cost ?? 0) }
    for (const c of contracts) for (const a of c.s.attempts ?? []) add("builder", short(a.model), a.cost, a.duration_ms, c.id)
    for (const r of Object.values(research?.results ?? {})) if (r.model) add("researcher", short(r.model), r.cost, r.duration_ms, null)
    return total || Object.keys(byModel.builder).length || Object.keys(byModel.researcher).length ? { total, byRole, byModel, byContract } : null
  })()
...
export function renderBoard(args) {
  ...
  const strip = `<div class="strip">
    ...
    <div><div class="k">Supervisor</div><div class="v">tú</div><div class="sub">la sesión de Claude Code de este proyecto</div></div>
    ...
    <div><div class="k">Coste (equiv. API)</div><div class="v">${costs ? usd(costs.total) : "–"}</div><div class="sub">${costs ? esc(Object.entries(costs.byRole).map(([r, c]) => `${r} ${usd(c)}`).join(" · ")) : "sin intentos todavía"}</div></div>
  </div>`
  ...
  const modelsTab = `<div class="strip s3"><div><div class="k">Escalera builder</div><div class="v sm">${esc((ladders.builder ?? []).map(short).join(" › "))}</div></div><div><div class="k">Escalera researcher</div><div class="v sm">${esc((ladders.researcher ?? []).map(short).join(" › "))}</div></div><div><div class="k">Regla</div><div class="v sm">2 intentos por peldaño · ${models?.max_models_per_item ?? 3} peldaños por ítem · si un modelo te falla seguido, bájalo en models.json</div></div></div>
    ...
    <div class="q" style="padding:8px 16px 12px">Intentos y tasa salen de report.json; coste (equivalente API), sesiones y tiempo del evento final de cada llamada a claude -p.</div>`
