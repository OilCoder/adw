...
//   node .claude/codegen.mjs build [--parallel N (default 4; the supervisor passes 8)] [--only c1,c2] [--resume]
//   research and build run in the foreground: the supervisor's Bash tool runs
//   them in the background and wakes the session when they exit.
...
// `claude -p` on the user's subscription (lib/agent.mjs).
//
...
// launched this command in the background: its Bash tool wakes it with the
// whole output when the process exits, so the news only needs to be in the
// output (a `[codegen]` line) and in the journal, where the board shows it.
function notify(text) {
  journal({ kind: "notify", text, delivered: true })
  log(`[codegen] ${text}`)
}
...
async function writeBoard() {
  ...
  const boardFile = path.join(ROOT, ".claude", "lib", "board.mjs")
  let renderBoard
  let boardJson
  try {
  ...
  // as a local file and cannot read them itself).
  const reports = {}
...
async function buildOne(entry, { runDir, integration, integrationDir, state, report }) {
  ...
  // travel with the claude -p call instead (sandbox.mjs drops them before
  // the base commit, so the diff that lands never mentions them).
...
function printBuild(r) {
  ...
  if (passed < Object.keys(r.contracts).length)
    console.log(
      `Continue this run after fixing contracts with: node .claude/codegen.mjs build --resume [--only ids]`,
    )
  if (passed)
    console.log(
      `Land it with: node .claude/codegen.mjs merge${passed < Object.keys(r.contracts).length ? " --partial" : ""}`,
    )
}
...
try {
  if (command === "research") await research(args)
    ...
    const tree = git(["ls-files"])
      .split("\n")
      .filter((f) => f && !/^(\.codegen|\.claude|docs|wiki|data)\//.test(f) && f !== "CLAUDE.md")
    ...
    console.log(
      "usage: node .claude/codegen.mjs research [--only ids] [--reject ids] | build [--parallel N] [--only ids] [--resume] | status | structure | board [--watch] | merge [--partial]",
    )
    process.exit(2)
  }
} catch (e) {
