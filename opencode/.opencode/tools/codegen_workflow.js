...
export default tool({
  description:
    "Run the controlled code-generation workflow. Draft a Goal from user intent; deliberate an open Goal (research pending questions, obtain opinions on blocking questions with options, fold the evidence into the Goal); revise the Goal with the user's answers; approve the existing Goal after explicit user confirmation; orchestrate an approved Goal (on the planned route, or whenever the plan contradicts the Goal's triage, it stops as PLAN_REVIEW_REQUIRED with a PLAN.md for the user to review; call orchestrate again with `plan` set to the reviewed plan path to build); merge the integration branch of a completed run into the user's branch (fast-forward only) when the user explicitly asks.",
  args: {
