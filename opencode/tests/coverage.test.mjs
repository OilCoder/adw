...
test("PLAN.md shows the triage against the Goal's labels", async () => {
  const { plan, goal } = await fixtures()
  ...
  const markdown = renderPlanMarkdown(plan, lowGoal, { route: "planned", riskFloors })
  ...
  const clean = renderPlanMarkdown((await fixtures()).plan, goal, { route: "planned", riskFloors })
