...
| Ruta directa (§8.1) | Implementado: el Planner recibe `--route direct` y pide un contrato; si necesita más, la corrida se reencamina a planificada y pausa | `lib/orchestrator.mjs`, `scripts/run-planner.mjs`. El tope de dos intentos se impone desde 2026-09-08 (`config/budgets.json`, recorte al sellar) |
...
| Presupuestos (§9) | Implementado con techos y suelos del sistema desde 2026-09-08 (B2) | `config/budgets.json`: Goal (`applyGoalLimits` en `lib/goal.mjs`, aplicado por `run-goal.mjs` en draft, revise y approve; suelo `max_research_calls ≥ preguntas required pendientes`) y contrato (`applyPlanLimits` en `plan-validation.mjs`, aplicado por el orquestador al sellar; `max_builder_attempts` ≤ 2 en directa, ≤ 3 en planificada). El código recorta, no rechaza; cada ajuste va al resumen, a GOAL.md / PLAN.md y a `events.jsonl` (`BUDGET_ADJUSTED`) |
...
- Configuración del sistema en `.opencode/codegen/config/`: `model-pools.json` (registro de modelos), `budgets.json` (techos y suelos de presupuestos), `risk-floors.json` (riesgo mínimo por rutas; editable por proyecto).
