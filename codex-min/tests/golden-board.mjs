...
// fixed cost facts. The HTML must match byte for byte: a redesign changes it
// on purpose and then regenerates with --update. `sup` is what the supervisor's
// Codex thread looks like (read from ~/.codex/sessions by codex-db.mjs); the
// `idle` case has none (sup null, costs summed from the attempts).
...
const { renderBoard, boardJson } = await import(path.join(SRC, ".codex", "lib", "board.mjs"))
...
const models = JSON.parse(readFileSync(path.join(SRC, ".codex", "models.json"), "utf8"))
...
const supBusy = { sessionId: "ses_fixed", title: "Desarrollo profesional de visor de registros", slug: "quiet-knight", last: { at: T(1), kind: "tool", text: "bash: node .codex/codegen.mjs status" }, waiting: null, userMessages: [{ id: "m1", at: T(30), text: "resume el estado y dime qué falta" }] }
...
const cases = {
  ...
  idle: { ...(({ costs, ...rest }) => rest)(base), openai: null, alive: false, researchAlive: false, sup: null, merged: true },
}
