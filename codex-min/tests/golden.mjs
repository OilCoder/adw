...
// codegen.mjs against a fixture with the fake codex (no models, no cost) and
...
//               script: {…for the fake, see tests/fake-codex/codex},
//               runs: [[args…], …] }   each run is `node .codex/codegen.mjs args… --wait`.
...
for (const name of scenarios) {
  ...
  { const f = path.join(work, ".codex", "models.json"); const m = JSON.parse(readFileSync(f, "utf8")); for (const role of ["researcher", "builder"]) m[role] = m[role].flat(); writeFileSync(f, JSON.stringify({ ...m, ...(sc.models ?? {}), timeouts_seconds: { ...m.timeouts_seconds, ...(sc.models?.timeouts_seconds ?? {}) } }, null, 2)) }
  ...
  // HOME and CODEX_HOME point at the temp dir: no user config, no sessions, no rules.
  const env = { HOME: tmp, CODEX_HOME: path.join(tmp, "codex-home"), PATH: `${path.join(HERE, "fake-codex")}:${process.env.PATH}`, FAKE_CODEX_SCRIPT: scriptFile, FAKE_CODEX_STATE: state, TZ: "UTC", GIT_AUTHOR_NAME: "golden", GIT_AUTHOR_EMAIL: "g@l", GIT_COMMITTER_NAME: "golden", GIT_COMMITTER_EMAIL: "g@l" }
  const out = {}
  let runs = ""
  const record = (run, r) => {
  ...
  for (const entry of sc.runs) {
    ...
    if (Array.isArray(entry)) { record(run, spawnSync("node", [".codex/codegen.mjs", ...run, ...wait], opts)); continue }
    const main = spawn("node", [".codex/codegen.mjs", ...run, ...wait], opts)
    let out = ""
    ...
    for (const d of entry.during) {
      ...
      record([`(${d.after}s)`, ...d.run], spawnSync("node", [".codex/codegen.mjs", ...d.run], opts))
    }
    const status = await new Promise((r) => main.on("close", r))
    record(run, { stdout: out, stderr: "", status })
  }
  const status = spawnSync("node", [".codex/codegen.mjs", "status"], { cwd: work, encoding: "utf8", env: { ...process.env, ...env } })
  const structure = spawnSync("node", [".codex/codegen.mjs", "structure"], { cwd: work, encoding: "utf8", env: { ...process.env, ...env } })
  out["runs.txt"] = runs
  ...
  for (const [k, f] of [["research-status.json", ".codegen/research/status.json"], ["board.json", ".codegen/board.json"], ["calls.log", "../state/calls.log"]]) {
