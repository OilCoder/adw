...
// Credits, the unit of the Codex pricing page (models.json `prices`); not dollars.
export const usd = (n) => `${(n ?? 0).toFixed(2)} cr`
export const day = (t) =>
...
function header(d, { root, current, alive, researchAlive, now }) {
  ...
  const sess = sup?.title
    ...
    : `<div class="sub sess">${icon("bot")}Supervisor: tu sesión de Codex en este proyecto</div>`
...
function strip(d, { alive, researchAlive }) {
  ...
  const supText = sup?.last
    ...
    : "la sesión de Codex que lanza el script"
...
// ---- the quota: the ChatGPT subscription Codex uses (5 hours, week) ----
...
function quota(d, { now }) {
  ...
  const body = q
    ...
    : `<div class="empty" style="padding:0">Sin datos: sin sesión de Codex en ~/.codex/auth.json, token caducado o sin red.</div>`
  return `<div class="quota">${icon("gauge")}<div class="k">Cuota OpenAI${q?.plan ? ` · ${esc(q.plan)}` : ""}${q?.at ? ` · leída hace ${mins(now - Date.parse(q.at))}` : ""}</div>${body}</div>`
}
...
function modelsTab(d, { models, ladders = {} }) {
  ...
  return `${cards}${table("builder", "Builders", "contratos")}${table("researcher", "Researchers", "preguntas")}<div class="foot"><span>Intentos y tasa: report.json y research/status.json · coste, sesiones y tiempo: el uso de tokens de cada llamada a codex exec, a precio de models.json</span><span>Coste equivalente API en créditos · la suscripción no cobra por llamada</span></div>`
}
