...
// user's tree. The supervisor's configuration (AGENTS.md, .codex/) is left
// out before the base commit: Codex would load it into the builder and deny
// it its own files. The builder's own role goes in as the sandbox's AGENTS.md
// (`instructions`), inside the base commit so it never shows up as a change.
// Returns the integration commit it came from and the sandbox's base commit.
export function exportSandbox({ root, integration, wt, id, instructions }) {
  ...
  rmSync(path.join(wt, ".codex"), { recursive: true, force: true })
  rmSync(path.join(wt, "AGENTS.md"), { force: true })
  if (instructions) writeFileSync(path.join(wt, "AGENTS.md"), instructions + "\n")
  git(["init", "-q"], wt)
  git(["add", "-A"], wt)
  git(
    [
...
export function checkScope(contract, files) {
  const protectedPatterns = [".codegen/**", ".codex/**", "AGENTS.md", ...(contract.protected ?? [])]
