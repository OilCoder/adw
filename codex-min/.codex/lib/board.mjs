...
// `turn.completed` usage of every codex exec run, priced with models.json):
// API-equivalent credits, since the subscription charges nothing per call.
...
import { supervisorActivity } from "./codex-db.mjs"
...
// The two windows Codex shows (5 hours, week), read from the usage endpoint
// with the OAuth token Codex keeps in ~/.codex/auth.json (the same call
// opencode-min makes with OpenCode's copy of that token). One call per
// five minutes; null when there is no token, it expired, or there is no network.
...
// duration of its codex exec call. Summed by role, by model and by contract.
...
export function boardData(args) {
  ...
  // Tests pass a fixed `sup`; the script leaves it out and the board reads
  // the supervisor's TUI thread from Codex's session files.
  const sup = "sup" in args ? args.sup : supervisorActivity(args.root)
  ...
  // The OpenAI quota comes from codegen.mjs (one HTTP call, cached) or from a test.
...
export function boardJson(args) {
  ...
  return {
    ...
    supervisor: d.sup?.last
      ...
      : null,
    // Same shape as opencode-min's board.json (project-garden reads all three
    // harnesses alike): the Go and Claude quotas do not exist here and stay
    // null; a notice is delivered through `codex queue` (no port).
    quota: null,
    openai: d.openai,
    claude: null,
