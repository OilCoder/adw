#!/usr/bin/env node
// Minimal code-generation loop on Codex.
//
//   node .codex/codegen.mjs research [--only q1,q2] [--reject q3,q4] [--wait]
//   node .codex/codegen.mjs build [--parallel N (default 4; the supervisor passes 8)] [--only c1,c2] [--resume] [--wait]
//   research and build detach from the shell that launched them (Codex's shell
//   tool would wait on them); --wait keeps them in the foreground. Both must
//   run outside Codex's sandbox (escalated): the agents they spawn are
//   `codex exec` processes that need ~/.codex and the network.
//   node .codex/codegen.mjs status
//   node .codex/codegen.mjs structure          lists tracked files that break .codegen/structure.md
//   node .codex/codegen.mjs merge [--partial]     fast-forward the run's integration branch into the user's branch
//   node .codex/codegen.mjs board [--watch]     writes .codegen/board.html (also written on every state change)
//
...
//
// Models come from .codex/models.json, cheapest first, edited by hand: if a
// model keeps failing you, move it down there. Every agent runs through
// `codex exec` on the user's ChatGPT subscription (lib/agent.mjs).
//
...
import { supervisorThread } from "./lib/codex-db.mjs"
...
let MODELS, TIMEOUTS, STEP_CAP, AGENTS
try {
  ;({ models: MODELS, timeouts: TIMEOUTS, stepCap: STEP_CAP, agents: AGENTS } = loadModels(ROOT))
} catch (e) {
...
// Wakes the supervisor. The Codex TUI is a chat turn: nobody is awake between
// turns, so the script queues the news into the supervisor's thread with
// `codex queue` (the TUI session opened in this project, found in Codex's
// session files). The TUI delivers a queued message when its current turn
// ends. No thread: nothing is sent and the board still shows the state.
function notify(text) {
  let thread = null
  try {
    thread = supervisorThread(ROOT)?.id ?? null
  } catch {}
  const queued =
    thread &&
    spawnSync("codex", ["queue", "--thread", thread, "--message", `[codegen] ${text}`], {
      cwd: ROOT,
      stdio: "ignore",
      timeout: 15000,
    }).status === 0
  journal({
    kind: "notify",
    text,
    delivered: Boolean(queued),
    thread,
    reason: !thread ? "no supervisor session in this project" : !queued ? "codex queue failed" : undefined,
  })
  log(`[codegen] ${text}`)
  return Boolean(queued)
}

// research and build spawn `codex exec` agents, which write ~/.codex and talk
// to the network: inside Codex's sandbox they die at once (verified
// 2026-09-14: "failed to initialize in-process app-server client: Read-only
// file system"). So the command refuses to start there instead of failing
// one agent at a time; the supervisor runs it escalated (.codex/rules allows
// it without a prompt).
function refuseSandbox() {
  const home = process.env.CODEX_HOME ?? path.join(os.homedir(), ".codex")
  // No ~/.codex at all is not a sandbox (a sandbox has it, read-only).
  if (!existsSync(home)) return
  const probe = path.join(home, `.codegen-probe-${process.pid}`)
  try {
    writeFileSync(probe, "")
    rmSync(probe, { force: true })
  } catch {
    throw new Error(
      "~/.codex is not writable: this command is running inside Codex's sandbox. Run it again with escalated permissions (outside the sandbox); the agents it launches are codex exec processes.",
    )
  }
}

// Re-runs this command detached so the supervisor's shell tool returns at once.
function detach(command) {
  ...
  child.unref()
  console.log(
    `${command} started in background (pid ${child.pid}); follow with: node .codex/codegen.mjs status`,
  )
}
...
function sealAndCheckTracked() {
  ...
  const sealed = [".codegen", ".codex", "AGENTS.md", ".gitignore"].concat(
...
async function writeBoard() {
  ...
  const boardFile = path.join(ROOT, ".codex", "lib", "board.mjs")
  let renderBoard
  let boardJson
  let openaiQuota
  try {
    ;({ renderBoard, boardJson, openaiQuota } = await import(
  ...
  // as a local file and cannot read them itself); the OpenAI quota is one HTTP
  ...
  const openai = await openaiQuota()
  const args = {
    root: ROOT,
    sandboxes: SANDBOXES,
    reports,
...
async function researchOne(q, { runDir, runId, previous, results }) {
  ...
  const tryOne = async (model) => {
    ...
    // The researcher works in an empty git repository of its own: Codex's
    // sandbox makes the working directory the fence, and AGENTS.md (its
    // role) is only read at the root of a git repository. The report is
    ...
    const prompt = [
      ...
      `Write the report to ./report.md in the current directory. When you are done, your final message (your reply, not the report) must be exactly one line: \`DONE ./report.md\` or \`PARTIAL ./report.md <what is missing>\`.`,
    ]
      .filter(Boolean)
      .join("\n")
    ...
    const closed =
      ...
      // Models like to bold or quote their last line: strip that before reading it.
      /^DONE\b/.test(
        r.finalText
          .trim()
          .split("\n")
...
function loadPlan() {
  ...
  if (!map) throw new Error("plan: write .codegen/structure.md first (see .codex/instructions/structure.md)")
  const ids = new Set()
...
// the same contract on the same ladder fails the same way. Unlike opencode-min,
// nobody is woken mid-run here, so the run does not wait for a fix: the
// request must arrive while another contract is still building.
...
async function buildOne(entry, { runDir, integration, integrationDir, state, report }) {
  ...
  // The sandbox is the builder's project: the supervisor's AGENTS.md, config
  // and rules must not load into it (they would forbid editing src/ and hand
  // the builder the wrong role). sandbox.mjs drops them and writes the
  // builder's own AGENTS.md before the base commit, so the diff that lands
  // never mentions them; permissions are Codex's sandbox around the directory.
  const { integrationHead, base } = exportSandbox({
    root: ROOT,
    integration,
    wt,
    id,
    instructions: AGENTS.builder.system,
  })
  ...
  const judge = (a, dir, racing = false) => {
    log(
      `  ${id}: ${a.verdict} (${a.steps} steps, ${a.files.length} files${a.cost != null ? `, ${a.cost.toFixed(3)} cr` : ""}${racing ? `, ${short(a.model)}` : ""})`,
    )
    if (a.verdict === "PASS") {
      winner = dir
      won = a
...
function printBuild(r) {
  ...
  if (passed < Object.keys(r.contracts).length)
    console.log(
      `Continue this run after fixing contracts with: node .codex/codegen.mjs build --resume [--only ids]`,
    )
  if (passed)
    console.log(
      `Land it with: node .codex/codegen.mjs merge${passed < Object.keys(r.contracts).length ? " --partial" : ""}`,
    )
}
...
try {
  if (command === "research") await research(args)
    ...
    const map = loadStructure()
    if (!map) throw new Error("no .codegen/structure.md yet (see .codex/instructions/structure.md)")
    const tree = git(["ls-files"])
      .split("\n")
      .filter((f) => f && !/^(\.codegen|\.codex|docs|wiki|data)\//.test(f) && f !== "AGENTS.md")
    ...
    console.log(
      "usage: node .codex/codegen.mjs research [--only ids] [--reject ids] [--wait] | build [--parallel N] [--only ids] [--resume] [--wait] | status | structure | board [--watch] | merge [--partial]",
    )
    process.exit(2)
  }
} catch (e) {
