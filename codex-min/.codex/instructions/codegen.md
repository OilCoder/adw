...
`node .codex/codegen.mjs`. The supervisor writes the files; the script runs
the agents. Models for scripted agents come from `.codex/models.json`
(OpenAI models, cheapest first, edited by hand). Every agent is a
`codex exec` call on the user's ChatGPT subscription, in Codex's sandbox
around its own directory, with the role of `.codex/agents/<role>.md` as its
AGENTS.md; the supervisor is the session the user opened (`.codex/config.toml`
sets its model). Never spawn agents yourself: the script runs them. A ladder entry is a
...
- `.codegen/structure.md`: the map, required before `build` (rules in
  `.codex/instructions/structure.md`). Machine-readable part: under
  ...
  name gets the verdict `STRUCTURE`. `node .codex/codegen.mjs structure`
...
- `node .codex/codegen.mjs research [--only id,id] [--reject id,id]`: runs every
...
- `node .codex/codegen.mjs build [--parallel N, default 4] [--only id,id] [--resume]`:
...
- `node .codex/codegen.mjs status`: report of the current run plus the
  last log lines.
...
`.codegen/runs/<run>/<id>/`.
- `node .codex/codegen.mjs merge [--partial]`: fast-forwards the run's
