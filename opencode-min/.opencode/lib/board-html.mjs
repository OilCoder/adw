...
function header(d, { root, current, alive, researchAlive, now }) {
  ...
  const sess = sup?.title
    ...
    : `<div class="sub sess">${icon("bot")}Sin sesión de supervisor en este proyecto</div>`
...
${cell("Supervisor", "bot", sup?.last ? `hace ${mins(supAgo)}` : "–", esc(supText))}
...
function journalEntry(d, j) {
  ...
  switch (j.kind) {
    ...
    case "notify":
      return {
        cls: e.delivered ? "acc" : "bad",
        icon: e.delivered ? "bell" : "belloff",
        title: e.delivered ? "Aviso al supervisor" : "Aviso no entregado",
        meta: `${e.delivered ? "" : `${esc(e.reason ?? "sin sesión")} · `}${esc(cut(e.text, 160))}`,
      }
    case "seal":
...
function modelsTab(d, { models, ladders = {} }) {
  ...
  return `${cards}${table("builder", "Builders", "contratos")}${table("researcher", "Researchers", "preguntas")}<div class="foot"><span>Intentos y tasa: report.json y research/status.json · coste, sesiones y tiempo: base de datos de OpenCode, solo lectura</span><span>Coste equivalente API · no representa necesariamente un cobro real</span></div>`
}
