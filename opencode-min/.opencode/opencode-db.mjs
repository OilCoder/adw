...
export function supervisorActivity(root) {
  ...
  try {
    ...
    const sessions = db.prepare("select id, time_updated from session where directory = ? and parent_id is null and (agent is null or agent not in ('researcher','builder')) order by time_updated").all(root)
    if (!sessions.length) return null
    const session = sessions.at(-1)
    const ids = sessions.map((s) => s.id)
    const messages = db.prepare(`select id, session_id, data, time_created from message where session_id in (${ids.map(() => "?").join(",")}) order by time_created`).all(...ids)
    const roleOf = {}
    const userMessages = []
    ...
    const parts = db.prepare(`select session_id, message_id, data, time_created from part where session_id in (${ids.map(() => "?").join(",")}) order by time_created`).all(...ids)
    let last = null
    for (const p of parts) {
      const d = parse(p.data)
      if (roleOf[p.message_id] === "user") {
        if (d.type === "text") { const u = userMessages.find((x) => x.id === p.message_id); if (u && !u.text) u.text = d.text ?? "" }
        continue
      }
      if (p.session_id !== session.id) continue
      if (d.type === "tool") last = { at: p.time_created, kind: "tool", text: `${d.tool}: ${String(d.state?.input?.command ?? d.state?.input?.filePath ?? "").slice(0, 80)}` }
      else if (d.type === "text" && d.text?.trim()) last = { at: p.time_created, kind: "text", text: d.text.trim() }
    ...
    return { sessionId: session.id, last, waiting, userMessages: userMessages.filter((u) => u.text && !/^Continue if you have next steps/i.test(u.text)) }
