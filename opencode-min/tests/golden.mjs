...
for (const name of scenarios) {
  ...
  if (sc.models) { const f = path.join(work, ".opencode", "models.json"); const m = JSON.parse(readFileSync(f, "utf8")); writeFileSync(f, JSON.stringify({ ...m, ...sc.models, timeouts_seconds: { ...m.timeouts_seconds, ...(sc.models.timeouts_seconds ?? {}) } }, null, 2)) }
